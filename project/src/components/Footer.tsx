...
import { ArrowUp, Send, Mail, Linkedin, PhoneCall, Sparkles, CheckCircle2, MessageCircle, Instagram, Twitter, Phone, Edit3, Lock } from 'lucide-react';
...
export const Footer: React.FC<FooterProps> = ({ theme, onNavigate, onOpenAdminModal }) => {
  const { data, isAdmin, logoutAdmin } = useContent();
  ...
  return (
    <footer className="w-full px-4 sm:px-8 pt-16 pb-12 mt-20 relative z-10">
      <div className="max-w-7xl mx-auto rounded-[36px] bg-[#0f0a2e]/90 backdrop-blur-2xl border border-white/15 p-8 md:p-12 text-white shadow-[0_20px_60px_rgba(0,0,0,0.5)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 pb-12 border-b border-white/10">
          ...
          <div className="md:col-span-3 space-y-3">
            ...
            <ul className="space-y-2 text-sm text-slate-300">
              ...
              <li>
                <button onClick={() => onNavigate('business-analysis')} className="hover:text-[#5ce1e6] transition-colors">
                  تحلیل بیزینس
                </button>
              </li>
              <li>
        ...
        {/* Bottom Bar: Copyright + Admin Trigger + Scroll to top */}
        <div className="pt-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-slate-400">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-slate-300 flex items-center gap-1.5 transition-colors cursor-pointer"
            >
              <Edit3 className="w-3.5 h-3.5 text-amber-400" />
              <span>{isAdmin ? 'پیشخوان مدیریت (CMS)' : 'ورود به سیستم مدیریت (/admin)'}</span>
            </button>
          </div>
